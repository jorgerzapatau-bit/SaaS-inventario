...
export async function PUT(req: NextRequest, { params }: Params) {
    ...
    try {
        ...
        const exists = await prisma.cliente.findFirst({ where: { id, empresaId: user.empresaId } });
        if (!exists) return Response.json({ error: 'Cliente no encontrado' }, { status: 404 });
        const updated = await prisma.cliente.update({ where: { id }, data: { nombre, contacto, telefono, email, direccion, rfc: rfc?.toUpperCase() || null, razonSocial, codigoPostal, regimenFiscal, usoCFDI } });
        return Response.json(updated);
    } catch { return Response.json({ error: 'Error updating client' }, { status: 500 }); }
}

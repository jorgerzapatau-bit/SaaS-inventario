...
function RegularizarModal({
    ...
    // ── Fases: 'registros' → elegir cuáles | 'seleccion' → cómo asignar | 'existente' | 'nueva'
    const [fase, setFase] = useState<'registros' | 'seleccion' | 'existente' | 'nueva'>('registros');
    ...
    useEffect(() => {
        fetchApi(`/obras/${obraId}/regularizar`)
            .then((d: RegularizarData) => {
                ...
                setSeleccionados(new Set(d.registros.map(r => r.id)));
                // Pre-seleccionar primera plantilla elegible
    ...
    const todosSeleccionados = selCount === totalRegistros && totalRegistros > 0;
    const algunoSeleccionado = selCount > 0 && !todosSeleccionados;

    // Metros totales de los seleccionados
    const metrosSeleccionados = registros
        .filter(r => seleccionados.has(r.id))
        .reduce((s, r) => s + r.metrosLineales, 0);

    // IDs a enviar: si están todos seleccionados → undefined (backend asigna todos)
    // Si es selección parcial → envía los IDs
    const registroIdsPayload = (): string[] | undefined => {
        if (selCount === 0) return undefined;
        if (todosSeleccionados) return undefined;      // fallback: todos
        return Array.from(seleccionados);
    };

    const toggleTodos = () => {
        if (todosSeleccionados || algunoSeleccionado) {
    ...
    const labelCTA = () => {
        ...
        if (fase === 'existente') {
            return selCount > 0
                ? `Asignar ${selCount} registro${selCount !== 1 ? 's' : ''} seleccionado${selCount !== 1 ? 's' : ''}`
                : 'Asignar todos los registros';
        }
        return selCount > 0
            ? `Crear y asignar ${selCount} registro${selCount !== 1 ? 's' : ''}`
            : 'Crear y asignar todos';
    };

    // ── Handlers ──────────────────────────────────────────────────────────────
    const buildPayload = (extra: Record<string, unknown>) => {
        const ids = registroIdsPayload();
        return { ...extra, ...(ids ? { registroIds: ids } : {}) };
    };
    ...
    const handleAsignarExistente = async () => {
        ...
        try {
            const res = await fetchApi(`/obras/${obraId}/regularizar`, {
                method: 'POST',
                body: JSON.stringify(buildPayload({ plantillaId: plantillaSeleccionada })),
            });
    ...
    const renderRegistros = () => (
        <div className="px-6 py-5 space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-start gap-3">
                ...
                <div>
                    <p className="text-sm font-semibold text-amber-700">
                        {loadingDatos ? '…' : totalRegistros} registro{totalRegistros !== 1 ? 's' : ''} sin plantilla asignada
                    </p>
                    <p className="text-xs text-amber-600 mt-0.5">
                        Selecciona cuáles quieres regularizar ahora. Puedes dejarlo parcial y repetir el proceso después.
                    </p>
                </div>
            </div>
            ...
            {loadingDatos ? (
                ...
                <div className="text-center py-8 text-gray-400 text-sm">No hay registros sin plantilla.</div>
            ) : (
                <>
                    ...
                    <div className="flex items-center justify-between">
                        <label className="flex items-center gap-2 cursor-pointer select-none">
                            ...
                            <span className="text-xs font-semibold text-gray-600">
                                Seleccionar todos ({totalRegistros})
                            </span>
                        </label>
                    ...
                    {/* Lista de registros agrupada por equipo */}
                    {(() => {
                        // Agrupar por equipo
                        const grupos = new Map<string, RegistroSinPlantilla[]>();
                        for (const r of registros) {
                            const key = r.equipo.nombre + (r.equipo.numeroEconomico ?? '');
                            if (!grupos.has(key)) grupos.set(key, []);
                            grupos.get(key)!.push(r);
                        }
                        return (
                            <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                                {Array.from(grupos.entries()).map(([eqKey, regs]) => {
                                    const eqIds      = regs.map(r => r.id);
                                    const eqChecked  = eqIds.every(id => seleccionados.has(id));
                                    const eqIndeter  = eqIds.some(id => seleccionados.has(id)) && !eqChecked;
                                    const eqMetros   = regs.reduce((s, r) => s + r.metrosLineales, 0);
                                    const equipoLabel = regs[0].equipo.nombre
                                        + (regs[0].equipo.numeroEconomico ? ` (${regs[0].equipo.numeroEconomico})` : '');

                                    const toggleEquipo = () => {
                                        setSeleccionados(prev => {
                                            const next = new Set(prev);
                                            eqChecked
                                                ? eqIds.forEach(id => next.delete(id))
                                                : eqIds.forEach(id => next.add(id));
                                            return next;
                                        });
                                    };

                                    return (
                                        <div key={eqKey} className="border border-gray-100 rounded-xl overflow-hidden">
                                            {/* Sub-cabecera equipo */}
                                            <div className="flex items-center justify-between bg-blue-50/60 px-3 py-2 border-b border-gray-100">
                                                <label className="flex items-center gap-2 cursor-pointer select-none flex-1">
                                                    <input
                                                        type="checkbox"
                                                        checked={eqChecked}
                                                        ref={el => { if (el) el.indeterminate = eqIndeter; }}
                                                        onChange={toggleEquipo}
                                                        className="w-3.5 h-3.5 accent-blue-600 flex-shrink-0"
                                                    />
                                                    <span className="text-xs font-semibold text-blue-700 flex items-center gap-1">
                                                        <Wrench size={10} /> {equipoLabel}
                                                    </span>
                                                </label>
                                                <span className="text-xs text-gray-400 flex-shrink-0">
                                                    {regs.length} reg · {eqMetros.toFixed(1)} m
                                                </span>
                                            </div>
                                                ...
                                                {regs.map(r => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <span className={`text-xs font-semibold ${checked ? 'text-blue-600' : 'text-gray-500'}`}>
                    ...
                    {selCount > 0 && (
                        <div className="bg-blue-50 rounded-xl px-4 py-3 flex items-center justify-between">
                            <span className="text-xs font-semibold text-blue-700">
                                {selCount} de {totalRegistros} registros
                            </span>
                            <span className="text-xs font-bold text-blue-700">
                                {metrosSeleccionados.toFixed(1)} m seleccionados
                            </span>
                        </div>
                    )}
                </>
    ...
    const renderSeleccion = () => (
        <div className="px-6 py-5 space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center justify-between">
                <div>
                    <p className="text-xs font-semibold text-blue-700">
                        {selCount > 0 ? `${selCount} registro${selCount !== 1 ? 's' : ''} seleccionado${selCount !== 1 ? 's' : ''}` : 'Todos los registros'}
                    </p>
                    <p className="text-xs text-blue-500 mt-0.5">{metrosSeleccionados.toFixed(1)} m · {selCount} días de producción</p>
                </div>
                <button
                    onClick={() => { setFase('registros'); setError(''); }}
            ...
            <p className="text-sm text-gray-500">¿Cómo quieres asignarlos?</p>
            ...
            <div className="space-y-3">
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        {hayElegibles && !loadingDatos && <span className="ml-auto text-blue-400 text-lg">→</span>}
                    </div>
                </button>
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">Define los términos del contrato y asigna los registros seleccionados</p>
                        </div>
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                        <span className="text-sm font-semibold text-gray-800">Plantilla {p.numero}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                            p.status === 'ACTIVA' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                                        }`}>
                                    ...
                                    <div className="mt-1.5 w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                        <div className="h-full bg-blue-400 rounded-full transition-all" style={{ width: `${pctUsado}%` }} />
                                    </div>
                                    {p.fechaInicio && (
    ...
    const renderNueva = () => (
        <div className="px-6 py-5 space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-lg px-3 py-2 text-xs text-blue-600">
                Al guardar: se creará esta plantilla y se asignarán los <strong>{selCount > 0 ? selCount : 'todos los'}</strong> registros seleccionados.
            </div>
        </div>
    );

    // ── Navegación hacia atrás por fase ──────────────────────────────────────
    const volverFase = () => {
        setError('');
        if (fase === 'existente' || fase === 'nueva') setFase('seleccion');
        else if (fase === 'seleccion') setFase('registros');
    };

    const tituloFase: Record<typeof fase, string> = {
        registros: 'Seleccionar registros',
        seleccion: 'Regularizar registros',
        existente: 'Asignar a plantilla existente',
        nueva:     'Crear nueva plantilla',
    };

    // ── Stepper visual ────────────────────────────────────────────────────────
    const pasos = ['registros', 'seleccion', 'existente/nueva'] as const;
    const pasoActual = fase === 'registros' ? 0 : fase === 'seleccion' ? 1 : 2;
    ...
    return (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                {/* Header */}
                <div className="border-b border-gray-100 px-6 pt-5 pb-4 flex-shrink-0">
                    <div className="flex items-start justify-between gap-3 mb-3">
                        <div>
                            ...
                            <h2 className="text-lg font-bold text-gray-800">{tituloFase[fase]}</h2>
                        </div>
                    ...
                    <div className="flex items-center gap-1">
                        {['Registros', 'Destino', 'Confirmar'].map((label, i) => (
                            <div key={i} className="flex items-center gap-1 flex-1">
                                ...
                                <span className={`text-xs flex-shrink-0 ${i === pasoActual ? 'text-blue-600 font-medium' : 'text-gray-400'}`}>
                                    {label}
                                </span>
                ...
                <div className="overflow-y-auto flex-1">
                    ...
                    ) : fase === 'registros' ? renderRegistros()
                    ...
                    <div className="border-t border-gray-100 px-6 py-4 flex gap-2 flex-shrink-0">
                        {fase === 'registros' ? (
                            <>
                                ...
                                <button
                                    onClick={() => { setError(''); setFase('seleccion'); }}
                                    disabled={selCount === 0 || loadingDatos}
                                    className="flex-1 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg disabled:opacity-40 transition-colors"
                                >
                                    {selCount > 0
                                        ? `Continuar con ${selCount} registro${selCount !== 1 ? 's' : ''} →`
